...
class Face_Recognizer:
    ...
    def __init__(self):
        ...
        self.current_frame_face_reserve_check_in = []
                    ...
                    if len(faces) != 0:
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(current_frame_e_distance_list) < 0.4:
                                ...
                                if self.face_name_known_list[similar_person_num] not in self.all_face_occur:
                                    if self.face_name_known_list[similar_person_num] != "unknown":
                                        self.all_face_occur.append(self.face_name_known_list[similar_person_num])
                                         #print(q)
                            ...
                            for kk, d in enumerate(faces):
                                ...
                                if (d.right() + self.ww > 640) :
                                    color_rectangle = (255, 0, 0)
                                
                                elif(d.left() - self.ww < 0):
                                    color_rectangle = (255, 255, 0)

                                
                                    if self.face_name_known_list[similar_person_num] in self.all_face_occur:
                                       temp_all_face_occur = []
                                       app_copy.add_attendance(self.face_name_known_list[similar_person_num])
                                       for q in self.all_face_occur:
                                           if q != self.face_name_known_list[similar_person_num]:
                                              print(q)
                                              temp_all_face_occur.append(q)
                                       self.all_face_occur = temp_all_face_occur
    ...
    def run(self):
        ...
        cap.set(1, 240) 
        #cap.set(cv2.CAP_PROP_FPS, 60)                       # 640x480
        self.process(cap)
